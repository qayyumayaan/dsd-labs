module display (
    input logic [31:0] data_in,  // Input data to be displayed
    output logic [6:0] segments  // 7-segment display output
);

    // Extract the lower 4 bits to display a single digit (assuming hex display)
    logic [3:0] digit;
    assign digit = data_in[3:0];

    // Drive the segments based on the digit (hex-to-7-segment decoding)
    always_comb begin
        case (digit)
